...
type SignupPaymentRequiredData = {
  playerId: number;
  playerEmail: string;
...
function buildEmailHtml({
  ...
  return `
    <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; color: #1a1a1a;">
      <h2 style="margin-bottom: 4px;">Action Required: Secure Your Spot</h2>
      <p style="color: #555; margin-top: 0;">Hi ${recipientName},</p>
      <p style="color: #374151;">
        You've been shortlisted for <strong>${eventName}</strong>. To secure your slot,
...
export async function notifySignupPaymentRequired(data: SignupPaymentRequiredData): Promise<void> {
  ...
  const subject = `Action Required: Secure your spot for ${resolvedEventName}`;
  ...
  if (!result.ok) {
    console.error(`[email] notifySignupPaymentRequired failed for player_id=${playerId}:`, result.error);
  }
}
