  ...
  return `
    <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; color: #1a1a1a;">
      ...
      <p style="color: #374151;">
        You've been accepted to <strong>${eventName}</strong>.
      </p>
...
export async function notifySignupAccepted(data: SignupAcceptedData): Promise<void> {
  ...
  const subject = `You're In! Accepted to ${resolvedEventName}`;
  ...
  if (!result.ok) {
    console.error(`[email] notifySignupAccepted failed for player_id=${playerId}:`, result.error);
  }
}
