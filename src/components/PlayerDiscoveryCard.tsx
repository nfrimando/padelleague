      ...
      <div className="flex items-start justify-between gap-2">
        <div className="relative shrink-0">
          <img
            ...
            className="h-10 w-10 sm:h-14 sm:w-14 rounded-lg sm:rounded-xl object-cover ring-2 ring-white dark:ring-slate-800"
          />
        </div>
        <div className="min-w-0 flex-1">
          <div className="text-sm font-semibold tracking-tight text-slate-900 dark:text-slate-100 leading-snug line-clamp-2">
            {hasName ? player.name : "-"}
          </div>
          {hasNickname ? (
            <div className="mt-0.5 text-[11px] font-medium text-slate-600 dark:text-slate-300 truncate">
              {player.nickname}
            </div>
          ) : null}
        </div>
        <div className="rounded-lg border border-sky-200 dark:border-sky-900/80 bg-sky-50/80 dark:bg-sky-950/25 px-2 sm:px-2.5 py-1 text-right shrink-0">
          <div className="text-[10px] uppercase tracking-[0.12em] text-sky-700/90 dark:text-sky-300/85">
            Rating
          </div>
          <div className="mt-0.5 text-base sm:text-lg font-bold tabular-nums text-sky-900 dark:text-sky-200">
            {!loadingLifetimeMatches && cardRating != null ? (
              cardRating.toFixed(2)
            ) : (
              <span className="inline-block h-5 w-10 sm:w-12 rounded bg-sky-200/80 dark:bg-sky-900/60 animate-pulse" />
            )}
          </div>
        </div>
      </div>

      <div className="mt-3 rounded-xl border border-slate-200/90 dark:border-slate-700 bg-slate-50/80 dark:bg-slate-800/70 px-3 py-2.5">
        <div className="grid grid-cols-2 gap-x-3">
          <div className="min-w-0">
            ...
            {loadingLifetimeMatches ? (
              <div className="mt-1 h-6 w-10 rounded-md bg-slate-200/70 dark:bg-slate-700/70 animate-pulse" />
          ...
          <div className="min-w-0">
            ...
            {loadingLifetimeMatches ? (
              <div className="mt-1 h-4 w-16 rounded bg-slate-200/70 dark:bg-slate-700/70 animate-pulse" />
            ) : hasLatestMatchDate ? (
              <div className="mt-1 text-xs font-semibold text-slate-700 dark:text-slate-200 leading-snug">
